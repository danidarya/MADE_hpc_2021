...
double CalcMatMulTime_kij_my_optimisation(double * A, double * B, double * C, size_t N)
{
    struct timeval start, end;
    double r_time = 0.0;
    size_t i, j, k;
    ...
    for (k = 0; k < N; k++){
        ...
        for(i = 0; i < N; i++)
        {
            ...
            for(j = 0; j < N; j++)
                C[dummy+j] += A[dummy_2] * B[dummy_3+j];
        }
    }
    gettimeofday(&end, NULL);
...
// kij ordering  my naive optimization (useless for -O3)
    average_runtime = 0.0;
    for(int n=0; n<NRuns; n++)
    {
        runtimes[n]=CalcMatMulTime_kij_my_optimisation(&A[0], &B[0], &C[0], N);
        printf("runtime %lf seconds\n", runtimes[n]);
        average_runtime += runtimes[n]/NRuns;
    }
    printf("average runtime kij my optimisation %lf seconds\n", average_runtime);
    printf("---------------------------------\n");
